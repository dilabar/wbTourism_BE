...
<body id="page-top">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Add Detail Page of Destination</h1>

                    </div>

                    <form class="addpagefrm contact100-form validate-form" action="{{ route('updatePage') }}"
                        ...
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="destination_id" name="destination_id">
                                        @if (count($category) > 0)
                                            ...
                                            @foreach ($category as $b_item)
                                                <option value="{{ $b_item->_id }}" {{ $b_item->_id == $place_detail->category_id ? 'selected':'' }}>{{ $b_item->name }}
                                                </option>
                                            @endforeach
                                        @endif
                                    </select>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="district_code" name="district_code" onchange="OnDistrictChanged(this.value);">
                                        @if (count($district) > 0)
                                            <option value="" >-- SELECT --</option>
                                            @foreach ($district as $d)
                                                <option value="{{ $d->district_code }}" {{ $place_detail->district_code == $d->district_code?'selected':'' }} >
                                                    {{ $d->district_name }}
                                                </option>
                                            @endforeach
                                        @endif
                                    ...
                                    <select class="form-control" id="template_type" name="template_type">
                                        ...
                                        @if (count($template_type) > 0)
                                            @foreach ($template_type as $template_item)
                                                <option value="{{ $template_item->template_id }}" {{ $place_detail->template_id == $template_item->template_id ? 'selected':'' }}>
                        ...
                        <input type="hidden" name="id" value="{{ $place_detail->id }}" />
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <img class="img-responsive" id="blah"
                                    src="{{ asset('assets/img/no-img.png') }}" width="240" alt="your image" />
                            </div>
                        </div>
                    </form>
                </div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            if($('#destination_id').val() !=null ){
                getplaces();
                getTemplete($('#template_type').val());
            }
            ...
            $("#template_type").change(function() {
                ...
                if (id == 0) {
                    ...
                    $.ajax({
                        url: "{{ url('/admin/template/edit/getHtml') }}",
                        type: 'GET',
                        cache: false,
                        data: {
                            'id': $(this).val(),
                            'page_id': '{{ $place_detail->id }}',
                            _token: '{{ csrf_token() }}'
                        }, //see the $_token
                        datatype: 'json',
                        beforeSend: function() {
    ...
    <script>
        ...
        function showDiv(divId, element) {
            ...
            input = $('#' + ctwm).find('input');
            $('#' + sn + '-onlytext_' + sp).find('.content').append(input);
    ...
    <script>
        function OnDistrictChanged(v)
        {
            var district_code = $('#district_code').val()
            var category_id = $('#destination_id').val()
            if (category_id == 0) {
                // $('#destination_dropdown').hide();
                $('#page_dropdown').hide();
                $('#template_dropdown').hide();
