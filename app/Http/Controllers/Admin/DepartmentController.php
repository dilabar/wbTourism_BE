...
class DepartmentController extends Controller
{
    ...
    {
        return view('Admin/department/tender/create');
    }
    public function tenderSave(Request $request)
    {
        ...
        $rules = [
            ...
            'close_date' => 'required',
            'subject' => 'required',
            'authority' => 'required|in:Department,WBTDCL',
            'pdf' => 'required',

        ];
         $messages = [
            'tender_no.required' => 'The Name field is required',
            'pub_date.required'=>'The Publish Date field is required',
        ...
        $attributes['tender_no'] = $request->tender_no;
        $attributes['pub_date'] = $request->pub_date;
        $attributes['close_date'] = $request->close_date;
        $attributes['subject'] = $request->subject;
        $attributes['authority'] = $request->authority;
        $attributes['pdf'] = $request->pdf;
        ...
        $tags_array=array();
            if(!empty($request->tender_no)){
            array_push( $tags_array,$request->tender_no);
            array_push( $tags_array,'tender');
            }
        $model1=new Item();
        if($pdf_content=$request->file('pdf')){
            $img_data = file_get_contents($pdf_content);
            // $height = Image::make($pdf_content)->height();
            // $width = Image::make($pdf_content)->width();
            $extension = $pdf_content->extension();
            $mimeType=$pdf_content->getMimeType();
            $binary_pdf = new MongoBinary($img_data, MongoBinary::TYPE_GENERIC);
            // $model1->width= $width;
            // $model1->height= $height;
            $model1->extension= $extension;
            $model1->mimType=$mimeType;
            $model1->type='Pdf';
            $model1->image_type='tender';
            if(count($tags_array)>0){
            $model1->tags=$tags_array;
            }
            $model1->img_data=$binary_pdf;
            $model1->is_active=1;
            $model1->is_approved=1;
            $pdf_is_save=$model1->save();           
        }
        $model=new Item();
      
        if(!empty(trim($request->tender_no)))
        $model->tender_no=trim($request->tender_no);
        if(!empty(trim($request->pub_date)))
        $model->publishing_date=trim($request->pub_date);
        if(!empty(trim($request->close_date)))
        $model->close_date=trim($request->close_date);
        if(!empty(trim($request->subject)))
        $model->subject=trim($request->subject);
        if(!empty(trim($request->authority)))
        $model->authority=trim($request->authority);
        $model->pdf_obj_id=new MongoObjectId($model1->getKey()) ;
        $model->page_type='detail';
        $model->is_active=1;
        $model->is_approved=1;
        $model->type='tender';
        ...
        if($model->save()){
            ...
            return redirect("/admin/department/tender/create")->with('success', 'Tender Submitted successfully');
        }
    ...
    {
        return view('Admin/department/rti/create');
    }
    public function rtiSave(Request $request)
    {
      
        $rules = [
            'rti_letter_no' => 'required',
            'rti_letter_date' => 'required',
            'subject' => 'required',
            'pdf' => 'required',

        ];
         $messages = [
            'rti_letter_no.required' => 'The RTI Letter No field is required',
            'rti_letter_date.required'=>'The RTI Letter Date field is required',
            'subject.required'=>'The Subject field is required',
            'pdf.required'=>'The Pdf field is required',
        ];
        ...
        $attributes['rti_letter_no'] = $request->rti_letter_no;
        $attributes['rti_letter_date'] = $request->rti_letter_date;
        $attributes['subject'] = $request->subject;
        $attributes['pdf'] = $request->pdf;
        ...
        $tags_array=array();
            if(!empty($request->rti_letter_no)){
            array_push( $tags_array,$request->rti_letter_no);
            array_push( $tags_array,'rti');
            }
        $model1=new Item();
        if($pdf_content=$request->file('pdf')){
            $img_data = file_get_contents($pdf_content);
        ...
        if(!empty(trim($request->rti_letter_no)))
        $model->rti_letter_no=trim($request->rti_letter_no);
        if(!empty(trim($request->rti_letter_date)))
        $model->rti_letter_date=trim($request->rti_letter_date);
    ...
    {
        return view('Admin/department/circular/create');
    }
    public function circularSave(Request $request)
    {
        ...
        $rules = [
         ...
         $messages = [
            'circular_no.required' => 'The 	Circular No field is required',
            'circular_date.required'=>'The Date field is required',
            'subject.required'=>'The Subject field is required',
            'pdf.required'=>'The Pdf field is required',
        ];
        ...
        $attributes['circular_no'] = $request->circular_no;
        $attributes['circular_date'] = $request->circular_date;
        $attributes['subject'] = $request->subject;
        $attributes['pdf'] = $request->pdf;
        ...
        $tags_array=array();
            if(!empty($request->circular_no)){
            array_push( $tags_array,$request->circular_no);
            array_push( $tags_array,'circular');
            }
        $model1=new Item();
        if($pdf_content=$request->file('pdf')){
            $img_data = file_get_contents($pdf_content);
        ...
        if($model->save()){
            // dd('Gallery uploaded successfully');
            return redirect("/admin/department/circular/create")->with('success', 'Circular Submitted successfully');
        }
    ...
    {
        return view('Admin/department/notice/create');
    }
    public function noticeSave(Request $request)
    {
        ...
        $rules = [
         ...
         $messages = [
            'notice_no.required' => 'The notice_no No field is required',
        ...
        $attributes['notice_no'] = $request->notice_no;
        ...
        $tags_array=array();
            ...
            array_push( $tags_array,$request->notice_no);
            array_push( $tags_array,'notice');
            }
        $model1=new Item();
        if($pdf_content=$request->file('pdf')){
            $img_data = file_get_contents($pdf_content);
        ...
        if(!empty(trim($request->notice_no)))
        $model->rti_letter_no=trim($request->notice_no);
        if(!empty(trim($request->date)))
        $model->date=trim($request->date);
        ...
        if($model->save()){
            // dd('Gallery uploaded successfully');
            return redirect("/admin/department/notice/create")->with('success', 'Notice Submitted successfully');
        }
