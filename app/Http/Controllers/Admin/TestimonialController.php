...
class TestimonialController extends Controller
{
    ...
    public function show(Request $request)
    {
            ...
            if (!empty($request->search['value']))
              ...
              $serachvalue = '';
            $model=new Item();
            $query = $model::where('section_type', 'testimonials')->where('type', 'testimonial');
        ...
        return view('Admin/testimonial/listView',
        [
    ...
    public function delete(Request $request)
    {
        ...
        $attributes = array();
        $messages = array();
        $attributes['id'] = 'Testimonial Id';
        $attributes['cur_status'] = 'Current Status';
        $attributes['action_type'] = 'Action Type';
        $validator = Validator::make($request->all(), $rules, $messages, $attributes);
        if ($validator->passes()) {
            $id=new MongoObjectId($request->id);
            $row = Item::where('_id', $id)->where('section_type', 'testimonials')->where('type', 'testimonial')->first();
            if(empty($row)){
                return redirect("/admin/testimonial/list")->with('error', ' Testimonial Not Found');
            }
           ...
           $update_status=Item::where('_id', $id)->where('section_type', 'testimonials')->where('type', 'testimonial')->update($update_arr);
           $accpt_status=$accptreject_model->save();
           if($accpt_status &&  $update_status){
            ...
            return redirect("/admin/testimonial/list")->withErrors($validator);
        }
        if(empty($request->id)){
            return redirect("/admin/testimonial/list")->with('error', 'Testimonial Id Not Found');
        }
