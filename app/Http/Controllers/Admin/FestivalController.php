...
class FestivalController extends Controller
{
    ...
    { 
        ...
        array_push( $festival_array,$request->name);
        }
        $model1=new Item();
        if($thumbnail_image=$request->file('thumbnail_image')){
            ...
            $model1->festival_tag=$festival_array;
            }
        ...
        if($model->save()){
           // dd('Festival Uploaded successfully');
            return redirect("/admin/festival/list")->with('success', 'Festival Uploaded successfully');
        }
    ...
    public function show(Request $request)
    {
            ...
            if (!empty($request->search['value']))
              ...
              $serachvalue = '';
            $model=new Item();
            $query = $model::where('section_type', 'fest_event')->where('type', 'festival');
        ...
        return view('Admin/festival/listView',
        [
    ...
    public function delete(Request $request)
    {
        ...
        $attributes = array();
        $messages = array();
        $attributes['id'] = 'Festival Id';
        $attributes['cur_status'] = 'Current Status';
        $attributes['action_type'] = 'Action Type';
        $validator = Validator::make($request->all(), $rules, $messages, $attributes);
        if ($validator->passes()) {
            $id=new MongoObjectId($request->id);
            $row = Item::where('_id', $id)->where('section_type', 'fest_event')->where('type', 'festival')->first();
            if(empty($row)){
                return redirect("/admin/festival/list")->with('error', ' Festival Not Found');
            }
           ...
           $update_status=Item::where('_id', $id)->where('section_type', 'fest_event')->where('type', 'festival')->update($update_arr);
           $accpt_status=$accptreject_model->save();
           if($accpt_status &&  $update_status){
            return redirect("/admin/festival/list")->with('success', 'Festival Updated successfully');
            }
           
        }
        else {
            return redirect("/admin/festival/list")->withErrors($validator);
        }
        if(empty($request->id)){
            return redirect("/admin/festival/list")->with('error', 'Festival Id Not Found');
        }
