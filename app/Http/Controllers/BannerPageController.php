...
class BannerPageController extends Controller
{
    ...
    public function details(Request $request)
    {
        ...
        if(!empty($details_db->image_slider)){
        ...
        $img_content_banner=Item::where('_id', $details_db->banner_image)->first();
        if($img_content_banner){
        ...
        if($details_db->about_text  && count($details_db->about_text)>0){
        ...
        //    dd( $array);
            foreach($array as $about_item){
                ...
                if($about_item['text']!='onlytext' && $about_item['type']=='textwithimage'){
                    //-dd($about_item);
                    $content=Item::where('_id', $about_item['image_id'])->first();
                    $mim_type=$content->mimType;
                    $document = 'data:' . $mim_type . ';base64,' . base64_encode($content->img_data); 
        ...
        if($details_db->video_image){
            $video_image_db=Item::where('is_active', 1)->where('is_approved', 1)->where('_id', $details_db->video_image)->where('type', 'Image')->first();
            $video_image_type=$video_image_db->mimType;
            $video_img = 'data:' . $video_image_type . ';base64,' . base64_encode($video_image_db->img_data); 
            $details_db->video_image=$video_img;
        }
