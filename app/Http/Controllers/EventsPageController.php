...
class EventsPageController extends Controller
{
    ...
    {
        $btif_event_db = Item::where('type','gallery')->where('gallery_category_id',$id)->where('media_type','0')->get();
        $btif_event_list = collect([]);
        if (count($btif_event_db) > 0) {
            foreach ($btif_event_db as $gitem) {
                $data = collect();
                $content = collect([]);
                $document = collect([]);
                // $data->media_type = $gitem['media_type'] ? $gitem['media_type']:'';
                // $data->youtube_link = $gitem['youtube_link'] ? $gitem['youtube_link']:'';
