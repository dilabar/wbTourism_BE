...
class ItemController extends Controller
{
    ...
    { 
        ...
        $model->section_type='section5';
        if($model->save()){
            dd('Item uploaded successfully');
        }
    ...
    public function show(Request $request)
    {
            ...
            if (!empty($request->search['value']))
              ...
              $serachvalue = '';
            $model=new Item();
            $query = $model::where('section_type', 'tabs');
        ...
        return view('Admin/item/listView',
        [
    ...
    public function delete(Request $request)
    {
       ...
       $user_id = Auth::user()->user_id;
        ...
        $attributes = array();
        $messages = array();
        $attributes['id'] = 'Tab Id';
        $attributes['cur_status'] = 'Current Status';
        $attributes['action_type'] = 'Action Type';
        $validator = Validator::make($request->all(), $rules, $messages, $attributes);
        if ($validator->passes()) {
            $id=new MongoObjectId($request->id);
            $row = Item::where('_id', $id)->where('section_type', 'tabs')->first();
            if(empty($row)){
                return redirect("/admin/item/list")->with('error', ' Tab Not Found');
            }
           ...
           $update_status=Item::where('_id', $id)->where('section_type', 'tabs')->update($update_arr);
           $accpt_status=$accptreject_model->save();
           if($accpt_status &&  $update_status){
            return redirect("/admin/item/list")->with('success', 'Tab Updated successfully');
            }
           
        }
        else {
            return redirect("/admin/item/list")->withErrors($validator);
        }
        if(empty($request->id)){
            return redirect("/admin/item/list")->with('error', 'Tab Id Not Found');
        }
